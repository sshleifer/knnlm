...
class DimaServer:
    def __init__(self, path='dima.cfg', num_servers=4, num_clients=1, 
    save_dir='/checkpoint/sshleifer/faiss_idx',
    index_id='lang_en', ncentroids=4096):
    ...
    def add_vectors(self, embeddings, ids, embed_dim=1024, client_id=0, batch_size=1000):
        ...
        for i in tqdm(list(range(0, num_vec, batch_size))):
            ...
            client.add_index_data(self.index_id, emb, id.tolist())
            #single_client.add_index_data(index_id, embeddings, meta)
            # we added training data but did not start training yet
            #self.assertEqual(client.get_state(index_id), IndexState.NOT_TRAINED)
            if self.servers[0].get_ntotal(self.index_id) > 2e6  and client.get_state(self.index_id) == IndexState.NOT_TRAINED:
...
server = DimaServer(n_centroids=args.n_centroids)
start_time = time.time()
server.add_vectors(keys,  np.arange(args.start, args.dstore_size,), batch_size=args.bs)
